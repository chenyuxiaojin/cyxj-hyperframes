...
- 本文件位置:`cyxj/docs/`(重构时随 `cyxj/docs → 制作规范/docs` 一起搬走)
...
- `cyxj/assets/logos/` = 顶层 `assets/logos/` 的**重复镜像**。
...
引用成本(实测):规则层约 40 个文件需改写;`cyxj/` 被引用 146 次、`templates/` 176 次、`templates/components` 87 次、`assets/logos` 14 次。
...
│
├─ 制作规范/      你的生产大脑
│  ├─ docs/        ← cyxj/docs/*(HARD_CONSTRAINTS / REFERENCE_INDEX / ops / STYLE_BORROW_PLAYBOOK /
│  │                  ARCH_PARITY_REMOTION / OFFICIAL_DOCS_VALUE_INDEX / WORKLOG-* / lottie-davinci-experiment/ / 本 spec)
│  ├─ notes/       ← cyxj/notes/*(MY_VISUAL_DNA / MY_MOTION_NOTES / TEMPLATE_USAGE)
│  ├─ rules/       ← cyxj/rules/*(hyperframes/ + shared/)
│  └─ skills/
│     └─ cyxj-hyperframes-overlay/   ← cyxj/skills/cyxj-hyperframes-overlay
├─ 组件库/        可复用镜头组
│  └─ (cc-window / orbit-dots / pulse-bars / shake-error / spec-fill / text-effects / xcyj-tokens / COMPONENTS.json / README.md)
│                  ← templates/components/*
├─ 参考/
│  ├─ inspirations/      ← templates/inspirations/*(5 个转译参考)
│  ├─ catalog.json       ← templates/catalog.json(官方 catalog 索引)
│  ├─ hyperframes-launches/   原样不动(上游范例,自带 .git)
│  └─ 我的作品/          原样保留(删 overlays-only 一条,见 §5)
├─ 资源库/
│  ├─ logos/       ← assets/logos/*(34 个 SVG + LOGOS.md)
│  └─ 录屏/        ← 素材/录屏/*(gitignored,本地)
├─ 视频项目/      原样不动(在制 / 已发布)
...
| # | 现路径 | 新路径 | 方式 |
|---|---|---|---|
| 1 | `cyxj/docs/` | `制作规范/docs/` | `git mv` |
| 2 | `cyxj/notes/` | `制作规范/notes/` | `git mv` |
| 3 | `cyxj/rules/` | `制作规范/rules/` | `git mv` |
| 4 | `cyxj/skills/cyxj-hyperframes-overlay/` | `制作规范/skills/cyxj-hyperframes-overlay/` | `git mv` |
| 5 | `cyxj/scripts/lint-dead-css.sh` | `scripts/lint-dead-css.sh`(并入顶层 scripts) | `git mv` |
| 6 | `cyxj/README.md` | `制作规范/README.md` | `git mv` |
| 7 | `templates/components/` 全部 | `组件库/` | `git mv` |
| 8 | `templates/inspirations/` | `参考/inspirations/` | `git mv` |
| 9 | `templates/catalog.json` | `参考/catalog.json` | `git mv` |
| 10 | `assets/logos/` | `资源库/logos/` | `git mv` |
| 11 | `素材/录屏/` | `资源库/录屏/` | 普通 `mv`(gitignored) |
| 12 | 空壳 `cyxj/` `templates/` `assets/` `素材/` | 删除 | `rmdir` |

## 5. 删除清单(破坏性,已获用户确认)

- `cyxj/templates/`(components + inspirations 重复镜像)—— 真源在 §4 #7/#8。
- `cyxj/assets/logos/`(logo 重复镜像)—— 真源在 §4 #10。
...
4. 替换后**双向 grep 验证**:
   - 残留检查:规则层文件里不应再出现 `cyxj/`、`templates/`、`assets/logos`、`素材/`(除非是历史叙述,逐条人工确认)。
...
**脚本路径写死处**:
- `scripts/refresh-catalog.sh`:`templates/catalog.json` → `参考/catalog.json`(3 处)
- `scripts/refresh-zero-usage.sh`:`templates/catalog.json` → `参考/catalog.json`;`cyxj/docs/REFERENCE_INDEX.md` → `制作规范/docs/REFERENCE_INDEX.md`
- `scripts/sync-tokens.sh`:`templates/components/xcyj-tokens/...` → `组件库/xcyj-tokens/...`
- `scripts/hf-lint-hook.sh`:跳过模式 `*/templates/*` → `*/组件库/*`、`*/参考/*`(参考已在);`cyxj/docs/HARD_CONSTRAINTS.md` → `制作规范/docs/HARD_CONSTRAINTS.md`
...
- [ ] 残留 grep:规则层无遗留旧路径(`cyxj/` `templates/` `assets/logos` `素材/`)。
...
- 重构前先 commit 基线(§8.0)。本次重构若出问题:`git reset --hard <基线>` + 恢复 gitignored 的普通 mv(录屏可从原位置 git? 否,gitignored → 需手动备份;执行前对 `素材/录屏/` 与待删的 `overlays-only` 先确认用户不需要再删)。
