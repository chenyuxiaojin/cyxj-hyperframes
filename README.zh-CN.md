...
```
videos/                         10 条已归档视频源工程（每条有 README 讲技术选型）
templates/
  tutorial-8beat/               唯一从 0 设计的真模板（8-beat 教程结构）
...
skills/
  cyxj-new-video/               全流程 skill：建工程 → lint → preview → 归档
  cyxj-add-block/               从 catalog 装零件到当前工程
...
推荐用法是开一个 Claude Code 或 Codex 会话，说一句**「做个新视频，主题《XXX》」**。[`skills/cyxj-new-video/SKILL.md`](skills/cyxj-new-video/SKILL.md) 会接管整个流程：

1. 问形态 / 主题 / 时长
2. 读 [`docs/REFERENCE_INDEX.md`](docs/REFERENCE_INDEX.md) 推荐 2-3 个参考工程
3. 复制模板到 `YYYY-MM-DD/<slug>/`，更新 `meta.json` 和 `index.html`
4. 等你提供文案 → 填充 beats → lint → preview
5. 你说「做完了」→ 自动归档进 `videos/<日期>-<slug>/`

**Claude Code 用户**：skill 入口在 `.claude/skills/cyxj-{new-video,add-block}`  
**OpenAI Codex CLI 用户**：同一套 skill 软链在 `.agents/skills/cyxj-{new-video,add-block}`

两边都指向 `skills/` 下的同一份源。**改 skill 只在 `skills/` 改**。

---

## 对比同类工具

| | **cyxj-hyperframes** | **Remotion**（[cyxj-remotion](https://github.com/chenhuajinchj/cyxj-remotion)） | **Motion Canvas** | **HeyGen 官方 student-kit** |
|---|---|---|---|---|
...
HyperFrames：HTML + GSAP，上手门槛低，适合口播配动效的教程类视频。Remotion：React + TypeScript，适合数据驱动或程序化生成的视频。本仓库有姊妹仓 [`cyxj-remotion`](https://github.com/chenhuajinchj/cyxj-remotion) 对应 Remotion 管线。

**不会写代码能用吗？**  
可以。复制 `templates/tutorial-8beat/`，找每个 composition 文件里的 `{{占位符}}`，替换成你的文案。模板 README 里有完整的占位符清单。也可以把脚本粘给 Claude Code 让它填。

**中文字幕渲染有坑吗？**  
两个已知坑：(1) 不要用 `npx hyperframes transcribe` 跑中文音频，直接用 `whisper-cli`。(2) 无头 Chromium 渲染时中文字体偶发回退（Google Fonts CDN 超时）——把 woff2 文件本地化可避（参考 `2026-05-20/karpathy-anthropic/` 里的字体包）。完整细节：[`docs/HARD_CONSTRAINTS.md`](docs/HARD_CONSTRAINTS.md) §4 和 §8。

**OpenAI Codex 用户能用吗？**  
可以。skill 软链在 `.agents/skills/`，符合 OpenAI Agents 格式。新机器重建软链：`npx skills add heygen-com/hyperframes -y`。
...
| 顺序 | 看哪个 |
|---|---|
| 1. 先看成品 | [YouTube 上看](https://youtu.be/fnKGWHSd0NE)，或读 [`videos/2026-05-04-claude-19-tips/README.md`](videos/2026-05-04-claude-19-tips/README.md) |
| 2. 动效美学纪律 | [`docs/STYLE_BORROW_PLAYBOOK.md`](docs/STYLE_BORROW_PLAYBOOK.md) |
...
| 5. 看 10 条视频的设计思路 | [`videos/*/README.md`](videos) |
