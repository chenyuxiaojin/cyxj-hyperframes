...
XCYJ（陈与小金）的 YouTube 教程视频**生产工作台**——基于 HeyGen 的 [HyperFrames](https://hyperframes.heygen.com)（HTML + GSAP 渲染管线）做技术教程的片头、转场、整片演示。进入的第一步是读本文件，然后读 `official/` 和 `cyxj/`；不要用 `npx skills update` 管理 `official/`。
...
**关键架构区分**：
- `.agents/skills/` 和 `.claude/skills/` —— active discovery，只保留 `official/skills` 镜像入口和 `cyxj/skills/cyxj-hyperframes-overlay`
- `official/skills/` —— Codex plugin cache 镜像，不手改
- `cyxj/skills/` —— XCYJ overlay skill 真源
- `skills/` —— 历史 skill 源文件，旧入口已停用到 `skills-disabled-*`
...
|---|---|---|---|
| Claude Code | `CLAUDE.md` | `.claude/skills/` | official-* 软链 → `official/skills/`；XCYJ overlay → `cyxj/skills/` |
| Codex | `AGENTS.md` | `.agents/skills/` | official-* 软链 → `official/skills/`；XCYJ overlay → `cyxj/skills/` |

**单一源**：active XCYJ overlay skill 源文件在 `cyxj/skills/cyxj-hyperframes-overlay/`。Legacy workflow sources under `skills/` 暂保留，但 active links 已停用到 `skills-disabled-*`。
...
跨机器 clone 仓库后，`official/` 用 `cyxj/scripts/sync-official-from-codex-cache.sh` 从 Codex plugin cache 同步；`参考/hyperframes-launches/` 作为独立上游参考仓保留（gitignored，换机需单独 clone）。
...
| 路径 | 内容 |
|---|---|
| `cyxj/docs/REFERENCE_INDEX.md` | ⭐ 上游参考工程入口：18 工程 + 46 catalog 零件 + 9 skill 索引 |
| `assets/logos/` | ⭐ 33 个 AI 厂商 / 工具 SVG logo |
| `templates/tutorial-8beat/` | 8 beat 教程结构——已过冷启动验证的真模板（2026-06-11 修复 grain-overlay 裸片段） |
