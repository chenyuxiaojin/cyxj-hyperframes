...
├── cyxj/
│   └── skills/cyxj-hyperframes-overlay/   # Active XCYJ overlay
├── docs/
...
跨机器 clone 仓库后，`official/` 用 `cyxj/scripts/sync-official-from-codex-cache.sh` 从 Codex plugin cache 同步；`hyperframes-student-kit/` 和 `hyperframes-launches/` 仍作为独立上游参考仓保留。
