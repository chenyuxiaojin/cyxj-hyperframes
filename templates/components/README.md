...
- **单一真源**：`templates/components/`。`cyxj/templates/components/` 是**待废弃镜像**，新组件只进这里，不要在镜像里加。
- **官方 catalog 是另一套**：`templates/catalog.json` 是上游 HeyGen 官方零件（data-chart / 转场 / 特效…），与本自有组件库分离、互不混淆。
...
3. 历史已发布工程 `视频项目/已发布/`
4. 官方 `templates/catalog.json`

## 登记新组件（晋升仪式）

做完一条视频后，零件若满足晋升条件（被 ≥2 作品复用 / 体现品牌 DNA，判定见仓库根 `CLAUDE.md`「组件晋升规则」），按此登记：

1. 在 `templates/components/<id>/` 放 `<id>.html` + `<id>.css` + `README.md`（README 含：用途、最小用法、变体、依赖、来源工程）。
...
5. 不要再往 `cyxj/templates/components/` 镜像里复制（单一真源 = 本目录）。
