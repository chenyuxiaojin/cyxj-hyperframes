...
**总时长（SRT 真源校准，seg06+seg07 完成后实测）**：38+112+80+91+201+152+**206.6**+**85.5**+97+107+128 = **1298.1s ≈ 21:38**。**总和待 seg08-10 PLAN 时同步校准平衡**（seg06 +40.6s 含 306-309 桥接 / seg07 −34.5s 不含 340 桥接，net +6.1s）。

**注**：seg04 段长 201s（5:21→8:42 = 201s，旧表写 200s 是约整误差），字幕条修正为 104-177（第 177 条是 seg04 结论“把这一个模型更好的去使用”，第 178 条才进入 seg05）；seg06 旧估 166s → SRT 真源校准 **206.6s**（含 306-309 桥接段 SEC F，2026-05-22 PLAN gate G1 决议）；seg07 旧估 120s → SRT 真源校准 **85.5s**（字幕条 310-339，340 交给 seg08 SEC A 入场，2026-05-22 PLAN gate G1 决议）；其余 8 段段长跟旧表一致，待各自 PLAN 时按 SRT 校准。**起点 / 终点的精度按 SRT 真源校准** —— 写 PLAN-seg{NN} 时打开 `字幕/加入之后.srt` 查该段第一条字幕的开始时间码，作为该段在全片的起点真值。
...
- **2026-05-22**：seg07 ch7-1 预测 1 · 上下文集市 v1 完成（本次 commit，4 SEC + 85.5s）
