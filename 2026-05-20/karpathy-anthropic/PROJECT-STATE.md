...
| seg10 | ch8 outro | 19:54.366 | 21:35 | ~100s | 418-450 | 套壳是产品 + Antigravity 2.0 砍 IDE + 北京交流 | cutaway | ⬜ todo | — |

**总时长（旧估算）**：38+112+80+91+201+152+166+120+97+107+128 = 1292s = 21:32

**总时长（SRT 真源校准，seg06+seg07+seg08+seg09 完成后实测）**：38+112+80+91+201+152+**206.6**+**85.5**+**90.7**+**137.3**+~100 = **~1294.1s ≈ 21:34**。前 9 段（seg00-09）SRT 真源累计 1194.1s = 19:54.1 = 字幕 417 末 ✓ 对齐。**seg10 旧估 128s → SRT 真源 ~100s 待 PLAN 时校准**（旧 PROJECT-STATE 把 seg09→seg10 切点写成 19:24，SRT 真源是 19:54，所以 seg10 实际 ~100s）。

**注**：seg04 段长 201s（5:21→8:42 = 201s，旧表写 200s 是约整误差），字幕条修正为 104-177（第 177 条是 seg04 结论“把这一个模型更好的去使用”，第 178 条才进入 seg05）；seg06 旧估 166s → SRT 真源校准 **206.6s**（含 306-309 桥接段 SEC F，2026-05-22 PLAN gate G1 决议）；seg07 旧估 120s → SRT 真源校准 **85.5s**（字幕条 310-339，340 交给 seg08 SEC A 入场，2026-05-22 PLAN gate G1 决议）；**seg08 旧估 97s → SRT 真源校准 90.7s（含 1.6s SEC A 头部 buffer 消化字幕 339→340 的 1.4s 口播停顿 + 89.1s 字幕内容；起点 16:06.166 紧贴 seg07 末，2026-05-22 PLAN gate G1 决议）**；**seg09 旧估 107s → SRT 真源 137.3s（+30.3s · 旧 PROJECT-STATE 把 seg10 起点写成 19:24，SRT 真源是 19:54.366，所以 seg09 真实段长 137.3s · 含 0.5s SEC A buffer + 136.8s 字幕；起点 17:36.866 紧贴 seg08 末，2026-05-22 PLAN gate G1 决议）**；其余 1 段（seg10）段长跟旧表一致，待 PLAN 时按 SRT 校准。**起点 / 终点的精度按 SRT 真源校准** —— 写 PLAN-seg{NN} 时打开 `字幕/加入之后.srt` 查该段第一条字幕的开始时间码，作为该段在全片的起点真值。
...
- **2026-05-22**：seg09 ch7-3 预测 3 · 非开发者打包工具 v1 完成（commit `3d1892f`，9 SEC + 137.3s）
  ...
  - **v2 todo**（task #7）：SEC E 末段（cc-window + "强人所难" hot 大字）视觉重心偏下，需把 .e-cc-window/e-cc-x/e-qiang-headline/e-qiang-sub 4 个元素重新布局到画面中央
