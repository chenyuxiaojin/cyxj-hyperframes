...
> ⚠️ **Codex 镜像滞后说明（2026-05-07）**：
> 本 skill 落后 `.claude/skills/cyxj-new-video/SKILL.md` 数轮迭代——缺 A0 URL 分流 / A0.5 决策树 / `tutorial-8beat` 选项 / 阶段 D 风格借鉴 / 硬约束单源指向。
> 原 A1 第 1 题中 A/B/C 三个伪模板（host-overlay / host-overlay-alpha / demo-fullscreen）已于 2026-05-07 移到 `参考库/历史模板/`（仅档案保留），路径已更新；这三种形态请**从 0 写工程**。
> 冷启动做新视频请优先用 Claude Code 跑 `.claude/skills/cyxj-new-video`。
> Codex 端待下一轮独立任务「两边 SKILL 全量同步」修复。

### A1. 问清四件事（一次性，用 AskUserQuestion 多选）

1. **视频形态**（影响选哪个模板）：
   - A. 录屏出镜（直出 mp4）→ **从 0 写**（拓扑回查：`参考库/历史模板/host-overlay/`）
   - B. 录屏要在达芬奇精修（alpha 输出）→ **从 0 写**（拓扑回查：`参考库/历史模板/host-overlay-alpha/`）
   - C. 纯演示无录屏 → **从 0 写**（拓扑回查：`参考库/历史模板/demo-fullscreen/`）
...
照 `docs/hyperframes-official/getting-started/quickstart.md` 的最小骨架（meta.json + hyperframes.json + index.html + compositions/*.html + assets/）从 0 建。可参考 `参考库/历史模板/<对应拓扑>/` 看旧形态怎么拼，但**不要直接 cp**——那些目录的 id 没抽净、底色还是旧 DNA 深蓝。
...
- 改 `meta.json` 的 `id` 和 `name`：填 `xcyj-<slug>`、可读中文 name（从 0 写时直接填正确值）
- 改 `index.html`：
  - `<title>` 标签
